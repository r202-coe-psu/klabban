...
def main():
    ...
    default_path = Path(__file__).resolve().parents[2] / "/deployment/klabban/data/psu_eng_data.csv"
    path = default_path
    ...
    if db is not None:
        try:
            ...
            camp = db['refugee_camps'].find_one({"name": "คณะวิศวกรรมศาสตร์ มหาวิทยาลัยสงขลานครินทร์"})
        except Exception as e:
