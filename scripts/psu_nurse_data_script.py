...
def map_row_to_doc(row, created_by_id=None, camp_id=None):
    ...
    # fix date 26/11/2025
    registration_date = to_py_datetime(datetime.datetime(2025,11,26))
    age = row.get("อายุ")
    try:
...
def main():
    ...
    default_path = Path(__file__).resolve().parents[2] / "/deployment/klabban/data/psu_nurse_data.xlsx"
    path = default_path
    ...
    if db is not None:
        try:
            ...
            camp = db['refugee_camps'].find_one({"name": "คณะพยาบาลศาสตร์ มหาวิทยาลัยสงขลานครินทร์"})
        except Exception as e:
