...
def map_row_to_doc(row, created_by_id=None, camp_id=None):
    ...
    name = row.get("ชื่อ-สกุล (ไม่ต้องระบุคำนำหน้าชื่อ)")

    registration_date = to_py_datetime(row.get("วรวิทย์"))
    age = row.get("อายุ")
    try:
...
def main():
    ...
    default_path = Path(__file__).resolve().parents[2] / "/deployment/klabban/data/psu_data2.csv"
    path = default_path
    ...
    for i, row in df.iterrows():
        ...
        name_val = row.get("ชื่อ-สกุล (ไม่ต้องระบุคำนำหน้าชื่อ)")
